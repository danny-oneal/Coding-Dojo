...
function binarySearch(sortedNums, num, min = 0, max = sortedNums.length - 1) {
    if (min > max) {
        return false;
    }
    ...
    if (num === sortedNums[mid]) {
        ...
        return binarySearch(sortedNums, num, min, mid - 1);
    } else {
        return binarySearch(sortedNums, num, mid + 1, max);
    }
}
