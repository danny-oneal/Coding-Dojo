 ...
 */
function divideByThree(pokemons) {
 ...
 * @returns {null} no return
 */
function moreThanOneType(pokemons) {
  for (let pokemon of pokemons) {
 ...
 * @returns {null} no return
 */
function poisonType(pokemons) {
  for (let pokemon of pokemons) {
    if (pokemon.types.length === 1 && pokemon.types[0] === "poison") {
 ...
 * @returns {null} no return
 */
function flyingSecondType(pokemons) {
  for (let pokemon of pokemons) {
    if (pokemon.types.length > 1 && pokemon.types[1] === "flying") {
 ...
 * @returns {null} no return
 */
function reversedNamesOfPoisonPokemon(pokemons) {
  for (let pokemon of pokemons) {
    if (pokemon.types.length === 1 && pokemon.types[0] === "poison") {
